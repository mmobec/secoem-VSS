...
preserved real scenarios, solved once for the whole horizon. Every stage's
fixed decisions (x_1, ..., x_{t-1}) come from that ONE solve, broadcast
identically to every real scenario -- not from the dynamic chain's
independently-solved G_1 groups. A single classical EV value, applied
identically to every real scenario, trivially satisfies DA_bid_mono (equal
values satisfy both <= and >=), so no per-group construction or
curve-consistency remedy is needed here (see HG's ev_subproblem.py's
module docstring / VSSD_implementation_report.md for the full reasoning,
which applies unchanged to this branch).

Each EEV_t (t=2..6) is solved once as a genuine joint multi-scenario problem
over all real scenarios (RP's own model plus one extra fixing constraint --
...
def solve_classical_ev(rp_instance, rp_scenario_data, abstract_model):
    ...
    parameter replaced by its overall (unconditional) expectation over all
    preserved real scenarios -- not the dynamic chain's per-group conditional
    averages. Implemented by reusing build_ev_instance() with a single
    synthetic "group" whose omega spans every real scenario (weight = total
    probability).
    """
