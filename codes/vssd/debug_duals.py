...
binding at any (t,s) where RP's own optimal eDA_p+eDA_m is ~0.
...
Ported from the HG (15min_ren_same) branch's codes/vssd/debug_duals.py, with
(t,q,s) collapsed to (t,s) throughout (see ev_subproblem.py's module
docstring). Not part of the vssd/ package's normal flow. Usage:
...
def main():
    ...
    print("\n-- Locating (t,s) points where eDA_p+eDA_m ~= 0 --")
    near_zero = []
    for t in rp_instance.T:
        for s in rp_instance.S:
            D = value(rp_instance.eDA_p[t, s]) + value(rp_instance.eDA_m[t, s])
            if D < EPS:
                near_zero.append((t, s))
    total = len(list(rp_instance.T)) * len(list(rp_instance.S))
    print(f"{len(near_zero)} / {total} (t,s) points have eDA~=0")

    print("\n-- Checking duals of IM_bounds_3/4 and IB_pos_UB/IB_neg_UB at those points --")
    binding = []
    checked = 0
    for (t, s) in near_zero:
        ...
        for cname in ("IB_pos_UB", "IB_neg_UB"):
            ...
            d = rp_instance.dual.get(c, 0.0)
            if abs(d) > EPS:
                binding.append((cname, None, t, s, d))
