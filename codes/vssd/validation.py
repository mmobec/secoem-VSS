...
See VSSD_implementation_report.md §3.7 for the tolerance/handling table this
module implements, including why Proposition 4 is checked informationally only
(this model has stochastic objective coefficients -- prices -- so Prop. 4's
"deterministic objective coefficients" hypothesis does not hold) and why
EDEV_2 == EDEV_1 is an *expected* outcome (RM bids submitted alongside DA bids,
before DA clears -- see report §3.3), not a validation failure.
"""
...
def validate_propositions(rp_value, edev_by_stage, vssd_by_stage, tol=DEFAULT_TOL):
    ...
    for t in range(1, T):
        ...
        note = None
        if t == 1:
            note = "EDEV_2 == EDEV_1 expected (RM shares DA's information set, see report §3.3)"
        ok &= check_ok
        checks.append({
