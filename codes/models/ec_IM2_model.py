...
model.SsI = pyo.Set(
    initialize=lambda m: [
        ...
        for l in m.S
        for j in m.S
        if (pyo.value(m.lI[2,t, l]) <= pyo.value(m.lI[2,t, j]) and j != l)
    ]
)
# =============================================================================
...
# 8.4 System Imbalances
model.pIB_p = pyo.Var(model.T, model.S, within=pyo.NonNegativeReals,  bounds=(0, 1e3))   # positive imbalance
model.pIB_m = pyo.Var(model.T, model.S, within=pyo.NonNegativeReals,  bounds=(0, 1e3))   # negative imbalance
...
model.IB_pos_slack = pyo.Var(model.T, model.S,within=pyo.NonNegativeReals)
model.IB_neg_slack = pyo.Var(model.T, model.S, within=pyo.NonNegativeReals)
