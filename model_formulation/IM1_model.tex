...
\title{EC Model Formulation - Intraday Market 1}
...
This model is based on the implementation of the DA model. This model allows the EC to place bids into the first intraday market, using information that was revealed after the RM has cleared. The results of the DA and RM are taken as parameters. Similarly to the RM model, the DA results are defined as parameters. Additionally, the matched up- and down-reserve are set to parameters as well, and a penalty term is introduced for reserve that cannot be delivered. This model can be included in the modular architecture that allows the simulation of the EC participating in several markets by placing bids, using the most up-to-date information at that time.\\
...
$\lambda^{R,\text{pen}}_{t,\omega}$ (\verb|lR_penalty|) & penalty price applied to undelivered reserve at time $t \in \mathcal{T}$ under scenario $\omega \in \Omega$ [\euro/MWh]. \\    
...
\begin{tabularx}{\textwidth}{l l}
    \textbf{Sets:} \\
    	$\mathcal{SSI}_1$ (\verb|Ssi1|) & $\{(t, l, j) \in \mathcal{T}_1 \times \Omega \times \Omega \hspace{2mm} | \hspace{2mm} l \neq j, \hspace{2mm} \lambda^I_{1, t, l} \leq \lambda^I_{1, t, j}, \hspace{2mm}$ \\ 
	& Rather technical. Used to ensure monotonicity of first intraday market bids \\
	& w.r.t. intraday market price realisations. \\
	& For every $t \in \mathcal{T}_1$ provides the scenario pairs $(l, j)$  \\
	&  s.t. $\lambda^I_{1, t, l} \leq \lambda^I_{1, t, j}$ (order is important). \\
    & Notice that this set is only for the first intraday market, since the other ones are,\\
    & from this model's view, decision variables of their respective stages.
    \\
...
To build a bidding curve for every intraday market from the results obtained at each scenario, the quantities matched in IM1 have to be monotonically increasing w.r.t. the scenario prices. This is guaranteed with the following constraints (\verb|IM_bid_mono_1_rule|):
\begin{equation} \label{EQ:lI_bid}
	e^{IM}_{1, t, \omega} \leq e^{IM}_{1, t, \omega'} \quad \forall (t, \omega, \omega') \in \mathcal{SSI}.
\end{equation}
...
For intraday market variables \( e^{IM}_{i,t,\omega} \), non-anticipativity constraints are imposed only for markets where the price is not yet known at the time of bidding, namely \( i = 2, 3 \). For $i = 1$, they are recourse variables. The constraints are defined as follows:

For IM: (\verb|NAC_eIM_1|)
  \begin{equation} \label{EQ:NAC_eIM1}
  	e^{IM}_{1, t, \omega} = e^{IM}_{1, t, \omega'} \quad 
  	\forall t \in \mathcal{T}_1, \ 
  	\forall \omega \in \Omega, \ 
  	\forall \omega' \in \mathcal{C}_{SG^{IM}_1, \omega}, \ \omega \neq \omega'.
  \end{equation}
  and (\verb|NAC_eIM|)
\begin{equation} \label{EQ:NAC_eIM}
	e^{IM}_{i, t, \omega} = e^{IM}_{i, t, \omega'} \quad 
	\forall i \in \{2,3\}, \ \forall t \in \mathcal{T}_i, \ 
	\forall \omega \in \Omega, \ 
	\forall \omega' \in \mathcal{C}_{SG^{IM}_i - 1, \omega}, \ \omega \neq \omega'.
\end{equation}
...
\begin{note}
Observe that each of the nonanticipativity constraints \eqref{EQ:NAC_mUC}-\eqref{EQ:NAC_rDB} might define the same constraint multiple times. This is not necessary in the implementation of the model, and in fact, we do not include them multiple times in our implementation.
...
\sum_{\omega \in \Omega}
    ...
    \Bigl[
      ...
      \;-\;
      \lambda^{R,\text{pen}}_{t,\omega}
        \bigl(r^{U,\text{pen}}_{t,\omega} + r^{D,\text{pen}}_{t,\omega}\bigr)
...
\notag & & & \eqref{EQ:NAC_eIM}, \eqref{EQ:NAC_eIM1} & \text{NAC IM} \\
