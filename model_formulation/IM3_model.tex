...
\title{EC Model Formulation - Intraday Market 3}
...
This model is based on the implementation of the DA model. This model allows the EC to place bids into the third intraday market, using information that was revealed up to the gate closing of the third intraday market. The results of the DA, RM, IM1 and IM2 as well as some EC variables are taken as parameters. This model can be included in the modular architecture that allows the simulation of the EC participating in several markets by placing bids, using the most up-to-date information at that time. What makes the model for the  third IM special, is that IM3 is cleared on day D instead of D-1 like the other markets. This means that variables dealing with the Battery Energy Storage System, Flexible Demand and Imbalances are already known for the first 10 hours of the day. This is explained in detail below.\\
...
Although IM3 stretches only from 12-24h, we still defined the time periods over 24 hours for the model to be more in line with the others. 
...
\textbf{Variables:} \\
$fd_{t, \omega}$ (\verb|var_fd|) & flexible demand at $t \in \mathcal{T}_3$ under scenario $\omega \in \Omega$ (after flexibility is \\
& considered) [MWh]. \\
...
& demand $D_t$ [MWh]. \\
    ...
    \end{tabularx}\\
All Flexible Demand variables are parameters for $t \setminus \mathcal{T}_3$ as at the time of decision making,these variables will already have been disclosed.\\
...
\begin{tabularx}{\textwidth}{l l}
    ...
    \textbf{Variables:} \\
    $c_{t, \omega}$ (\verb|cV|) & BESS charge rate at time $t \in \mathcal{T}_3$ under scenario $\omega \in \Omega$ [MW]. \\
    & Defined only during time periods corresponding to intraday market 3 (IM3). \\
    $d_{t, \omega}$ (\verb|dV|) & BESS discharge rate at time $t \in \mathcal{T}_3$ under scenario $\omega \in \Omega$ [MW]. \\
    & Only active during IM3 periods. \\
    $id_{t, \omega}$ (\verb|idV|) & Binary variable indicating charge ($0$) or discharge ($1$) mode \\
    & at time $t \in \mathcal{T}_3$, scenario $\omega \in \Omega$. \\
    $\soc_{t, \omega}$ (\verb|socV|) & State of charge of BESS at time $t \in \mathcal{T}_0 \cap \mathcal{T}_3$ under scenario $\omega \in \Omega$. \\
	\\
\end{tabularx}
All BESS variables are parameters for $t \setminus \mathcal{T}_3$ as at the time of decision making,these variables will already have been disclosed.
\\
...
We assume that the wind and solar farms cannot contribute to provide reserve. The other elements of the energy community (Battery and Flexible Demand) can. This follows the approach by \textcite{Heredia et. al.}, where VPP's bid to reserve markets is limited by the battery capabilities only. \\
Note: Since the IM3 gate closes at 10:00 AM on day $D$, the variables below are treated as parameters for hours $t \in \{1,\dots,10\}$ because by the time the bid is submitted, they will have been realized.
...
The reserve market parameters mentioned before already  abide to the constraints imposed in the previous stages models, so we impose that the undelivered reserve variables are nonnegative (\verb|at variable definition|) for the new variables.
\begin{equation} \label{EQ:RVarDef}
...
\begin{tabularx}{\textwidth}{l l}
    \textbf{Sets:} \\
    	$\mathcal{SSI}_3$ (\verb|Ssi_3|) & $\{(t, l, j) \in \mathcal{T}_3 \times \Omega \times \Omega \hspace{2mm} | \hspace{2mm} l \neq j, \hspace{2mm} \lambda^I_{3, t, l} \leq \lambda^I_{3, t, j}, \hspace{2mm}$ \\ 
	& Rather technical. Used to ensure monotonicity of intraday market bids \\
	& w.r.t. intraday market price realisations. \\
	& For every $t \in \mathcal{T}_3$ provides the scenario pairs $(l, j)$ in the previous stage \\
	& cluster s.t. $\lambda^I_{3, t, l} \leq \lambda^I_{3, t, j}$ (order is important).\\
    & Notice that this set is only for the third intraday market, since the other ones are\\
    & already cleared.
    \\
...
$e^{IM}_{i, t, \omega}$ (\verb|eIM|) & energy matched by the EC at intraday markets $i \in \{1, 2\}$ at time $t \in \mathcal{T}$ \\
...
\\
\textbf{Variables:} \\
$e^{IM}_{3, t, \omega}$ (\verb|eIM|) & energy matched by the EC at intraday market $i = 3$ at time $t \in \mathcal{T}_3$ \\
& under scenario $\omega \in \Omega$ [MWh]. \\
...
To build a bidding curve for the third intraday market from the results obtained at each scenario, the quantities matched in intraday markets have to be monotonically increasing w.r.t. the scenario prices. This is guaranteed with the following constraints (\verb|IM_bid_mono_3_rule|):
\begin{equation} \label{EQ:lI_bid}
	e^{IM}_{3, t, \omega} \leq e^{IM}_{3, t, \omega'} \quad \forall (t, \omega, \omega') \in \mathcal{SSI}.
\end{equation}
...
For IM: (\verb|NAC_eIM_3|)
  \begin{equation} \label{EQ:NAC_eIM3}
  	e^{IM}_{3, t, \omega} = e^{IM}_{3, t, \omega'} \quad 
  	\forall t \in \mathcal{T}_3, \ 
  	\forall \omega \in \Omega, \ 
  	\forall \omega' \in \mathcal{C}_{SG^{IM}_3, \omega}, \ \omega \neq \omega'.
  \end{equation}
...
\end{equation}
% ==========================
% Non-Anticipativity: BESS Operation
% ==========================

For BESS operational variables, we have (\verb|NAC_cV|):
\begin{equation} \label{EQ:NAC_cV}
	c_{t, \omega} = c_{t, \omega'} \quad \forall t \in \mathcal{T}_3, \ \forall \omega \in \Omega,\
	\forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega},\ \omega \neq \omega',
\end{equation}
and (\verb|NAC_dV|):
\begin{equation} \label{EQ:NAC_dV}
	d_{t, \omega} = d_{t, \omega'} \quad \forall t \in \mathcal{T}_3, \ \forall \omega \in \Omega,\
	\forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega},\ \omega \neq \omega',
\end{equation}
and (\verb|NAC_idV|):
\begin{equation} \label{EQ:NAC_idV}
	id_{t, \omega} = id_{t, \omega'} \quad \forall t \in \mathcal{T}_3, \ \forall \omega \in \Omega,\
	\forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega},\ \omega \neq \omega',
\end{equation}
and (\verb|NAC_socV|):
\begin{equation} \label{EQ:NAC_socV}
	\soc_{t, \omega} = \soc_{t, \omega'} \quad \forall t \in \mathcal{T}_3, \ \forall \omega \in \Omega,\
	\forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega},\ \omega \neq \omega'.
\end{equation}
and (\verb|NAC_rU_B|)
...
\sum_{\omega \in \Omega}
    ...
    \Bigr] \label{EQ:RESincome} \\
		...
		& + \sum_{t \in \mathcal{T}_3} \sum_{\omega \in \Omega} \Pi_{\omega} \lambda^{IB+}_{t, \omega} p^{IB+}_{t, \omega} \label{EQ:PosIBcollectRights} \\
		& - \sum_{t \in \mathcal{T}_3} \sum_{\omega \in \Omega} \Pi_{\omega} \lambda^{IB-}_{t, \omega} p^{IB-}_{t, \omega} \label{EQ:NegIBPayObligations} \\
		& - \sum_{t \in \mathcal{T}_3} \sum_{\omega \in \Omega} \Pi_{\omega} C^{FD} (afd^+_{t, \omega} + afd^-_{t, \omega}). \label{EQ:DemFlexPenalty}
	\end{align}
	\label{EQ:EECSW}
\end{subequations}


The second term \eqref{EQ:RESincome} represents the EC's expected income from the reserve market. The third term \eqref{EQ:IMincome} represents the EC's expected income from intraday markets. The forth term \eqref{EQ:PosIBcollectRights} represents the EC's expected positive imbalances collection rights. The fifth term \eqref{EQ:NegIBPayObligations} represents the EC's expected imbalance payment obligations. The last term \eqref{EQ:DemFlexPenalty} is a penalty on the use of demand flexibility. Since flexible demand decisions and imbalance setllements from hours 1-12 are not subject to optimization and are populated by default variables, they are not part of the objective function as to not pollute it with noise and make it easier to compare with the objective functions of the previous market's models.
...
\notag & & & \eqref{EQ:NAC_eIM3}, & \text{NAC IM} \\
