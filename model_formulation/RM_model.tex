...
\title{EC Model Formulation - Reserve Markets}
...
This model is based on the DAM model implementation. It expands that model to let the EC place bids into the reserve market, after the DAM has cleared. The results of the DAM are taken as parameters into the model and an up-to-date scenario tree is used, in which DA results are fixed. While the model is largely the same as the DAM model, there are some key differences. it can be seen that what previously were variables in the DA section, are now parameters. Additionally a new ordered set is defined for RM as well and the NAC regarding DA are removed.\\
The goal is to include these sub-models in a coding framework that can simulate a full market run of the EC, where the markets are cleared back-to-back and bids are made with the information that is revealed after each run.
...
Since the values of these parameters are obtained from running the DA model, it is assumed that they obey to all constraints formulated in that model, thus they do not need to be listed in the RM model as well.
...
We model the participation of the EC in intraday markets with a unique price-accepting bid for all scenarios. This is different to the modelling of reserve, where we are interested in obtaining reserve bid curves, which require a more detailed modeling.
...
\begin{equation} \label{EQ:NAC_pVRU}
	r^U_{t, \omega} = r^U_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{2, \omega}, \ \omega \neq \omega',
\end{equation}
(\verb|NAC_rU_B|)
\begin{equation} \label{EQ:NAC_rUB}
	r^{U, B}_{t, \omega} = r^{U, B}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{2, \omega}, \ \omega \neq \omega',
\end{equation}
and (\verb|NAC_rU_FD|)
\begin{equation} \label{EQ:NAC_rUFD}
	r^{U, FD}_{t, \omega} = r^{U, FD}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{2, \omega}, \ \omega \neq \omega'.
\end{equation}

For downward reserve variables, we have (\verb|NAC_rD|)
\begin{equation} \label{EQ:NAC_pVRD}
	r^D_{t, \omega} = r^D_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{2, \omega}, \ \omega \neq \omega',
\end{equation}
(\verb|NAC_rD_B|)
\begin{equation} \label{EQ:NAC_rDB}
	r^{D, B}_{t, \omega} = r^{D, B}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{2, \omega}, \ \omega \neq \omega',
\end{equation}
and (\verb|NAC_rD_FD|)
\begin{equation} \label{EQ:NAC_rDFD}
	r^{D, FD}_{t, \omega} = r^{D, FD}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{2, \omega}, \ \omega \neq \omega'.
\end{equation}
...
\begin{note}
Observe that each of the nonanticipativity constraints \eqref{EQ:NAC_pVRU}-\eqref{EQ:NAC_socV} might define the same constraint multiple times. This is not necessary in the implementation of the model, and in fact, we do not include them multiple times in our implementation.
...
\begin{subequations}
	...
	\label{EQ:EECSW}
\end{subequations}
The first term \eqref{EQ:DAincome} is from the DAM definition. Since the cleared prices are known, this term now does not represent the expected, but rather the realized EC DA income. The term is kept in the model definition to allow for comparisons of the models at different stages. The expected to the realized outcome can be compared.
