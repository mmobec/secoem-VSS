...
\section{Original model adapted to 15 minute periods}

\subsection{Introduction}

In the following sections we present the proposed problem formulation with a shift of time periods from 1h to 15-minute periods. This involves having a stage every 15-minutes (plus additional stages for the different markets). Since it is quite a large model, the sets, parameters, variables, and constraints are defined on separate sections.

\subsection{Fundamental Elements}
\label{SEC:FundamentalElements1}

\begin{tabularx}{\textwidth}{l X}
    \textbf{Sets:} \\
    $\mathcal{T}$ (\texttt{T}) & set of $T$ time periods $\{ 1, \dots, T \}$ [15-min periods]. \\
    $\mathcal{T}_0$ (\texttt{T0}) & extended set of time periods $\{ 0, \dots, T \}$ [15-min periods]. \\     
    $\mathcal{I}$ (\texttt{IM}) & set of $I$ intraday markets $\{1, \dots, I \}$. \\    
    $\mathcal{T}_i$ (\texttt{TIM}) & set of time periods in intraday market $i \in \mathcal{I}$ [15-min periods]. \\
    $\mathcal{I}_t$ (\texttt{IMT}) & set of intraday markets where $t \in \mathcal{T}$ is a time period. \\    
    $\Omega$ (\texttt{S}) & set of scenarios. \\
    $\mathcal{SG}$ (\texttt{SG}) & set of stages $\{1, \dots, nSG \}$. \\
    $\mathcal{SG}_0$ (\texttt{SG0}) & extended set of stages $\{0, \dots, nSG \}$. \\     
    ...
    \textbf{Parameters:} \\
    $T$ (\texttt{nT}) & number of time periods [15-min periods]. \\ 
    $\Delta T$ (\texttt{dT}) & duration of the period [hours]. \\
    $I$ (\texttt{nIM}) & number of intraday markets. \\
    $nSG$ (\texttt{nSG}) & number of stages. \\
    $SG^{IM}_i$ (\texttt{sgim}) & stage in which the price of intraday market $i \in \mathcal{I}$ is revealed. \\
...
\begin{equation} \label{EQ:FlexDemandPos1}
	fd_{t, \omega} \geq 0, \ afd_{t, \omega}^+ \geq 0, \ afd_{t, \omega}^- \geq 0 \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}

Flexible demand bounds:
\begin{equation} \label{EQ:FlexDemand_UB_preliminary1}
	fd_{t, \omega} \leq \overline{D}_t \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation} 
\begin{equation} \label{EQ:FlexDemand_LB_preliminary1}
	\underline{D}_t \leq fd_{t, \omega} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
Total daily demand (\verb|DailyDem|): 
\begin{equation} \label{EQ:DailyDem1}
	\sum_{t \in \mathcal{T}} fd_{t, \omega} = \sum_{t \in \mathcal{T}} D_t \quad \forall \omega \in \Omega.
\end{equation}

Certain percentage of the central demand needs to be met over a time interval (\verb|InterDem|):
\begin{equation} \label{EQ:InterDem1}
	\sum_{T_f^- \leq t \leq T_f^+} fd_{t, \omega} \geq c_f \sum_{T_f^- \leq t \leq T_f^+} D_t \quad \forall f \in \mathcal{F}, \ \forall \omega \in \Omega.
\end{equation}
Split of the variables $fd_{t, \omega}$ into its positive and negative parts (\verb|FlexDemDisplace|)
\begin{equation} \label{EQ:FlexDemDisplace1}
	D_t - fd_{t, \omega} = afd_{t, \omega}^+ - afd_{t, \omega}^- \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
...
\subsection{Battery Energy Storage System}
\label{SEC:BatteryEnergyStorageSystem1}
...
Nonnegativity  constraints:
\begin{equation} \label{dVdCPos1}
	c_{t, \omega} \geq 0, \quad d_{t, \omega} \geq 0, \quad id_{t, \omega} \in \{ 0, 1 \} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}

Discharging/charging bounds (\verb|VPP_state_dV| and \verb|VPP_state_cV|):
\begin{equation} \label{EQ:VPP_state_dV1}
	d_{t, \omega} \leq P^B id_{t, \omega} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:VPP_state_cV1}
	c_{t, \omega} \leq P^B (1 - id_{t, \omega}) \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}

Variable state of charge $\soc_{t, s}$ of the BESS (\verb|SOCV|): 
\begin{equation} \label{EQ:SOCV1}
	\soc_{t, \omega} = \soc_{t - 1, \omega} + \frac{(c_{t, \omega} - \frac{d_{t, \omega}}{\eta^B})\Delta T}{E^B} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega. 
\end{equation} 
Operational limits:
\begin{equation} \label{EQ:socVOperLimits1}
	\underline{\SOC} \leq \soc_{t, \omega} \leq \overline{\SOC} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}

Initial and final states of charge of the BESS (\verb|SOCV_ini| and \verb|SOCV_fin|):
\begin{equation} \label{EQ:SOCV_ini1}
	\soc_{0, \omega} = \SOC_0 \quad \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:SOCV_fin1}
	\soc_{T, \omega} = \SOC_T \quad \forall \omega \in \Omega.
\end{equation}

\subsection{Market Participation}
\label{SEC:MarketParticipation1}
...
\begin{equation} \label{EQ:DAVarDef1}
	e^{DA+}_{t, \omega} \geq 0, \ e^{DA-}_{t, \omega} \geq 0, \ ie^{DA+}_t, ie^{DA-}_t \in \{0, 1 \} \quad \forall t \in \mathcal{T} \quad \forall \omega \in \Omega.
\end{equation}
...
\begin{equation} \label{EQ:DA_sell_bid_LB1}
    \underline{E}^{DA} ie^{DA+}_{t, \omega} \leq e^{DA+}_{t, \omega} \leq (\overline{P}^W \Delta T + \overline{P}^{PV} \Delta T + P^B \Delta T - \underline{D}_t) ie^{DA+}_{t, \omega}  \quad \forall t \in \mathcal{T}, \quad \forall \omega \in \Omega.
\end{equation}
\begin{equation} \label{EQ:DA_buy_bid_LB1}
	\underline{E}^{DA} ie^{DA-}_{t, \omega} \leq e^{DA-}_{t, \omega} \leq (P^B \Delta T + \overline{D}_t) ie^{DA-}_{t, \omega} \quad \forall t \in \mathcal{T}, \quad \forall \omega \in \Omega.
\end{equation}

\begin{equation} \label{EQ:DA_buy_or_sell1}
	ie^{DA+}_{t, \omega} + ie^{DA-}_{t, \omega} \leq 1 \quad \forall t \in \mathcal{T}, \quad \forall \omega \in \Omega.
\end{equation}

Offer curves need to be monotone w.r.t. the scenario prices (\verb|DA_bid_mono_1| and \verb|DA_bid_mono_2|).
\begin{equation} \label{EQ:DA_bid_mono_11}
	e^{DA+}_{t, \omega} \leq e^{DA+}_{t, \omega'} \quad \forall (t, \omega, \omega') \in \mathcal{SSD}
\end{equation} 
\begin{equation} \label{EQ:DA_bid_mono_21}
	e^{DA-}_{t, \omega} \geq e^{DA-}_{t, \omega'} \quad \forall (t, \omega, \omega') \in \mathcal{SSD}.
\end{equation}
...
$T^R$ (\texttt{TSR}) & duration of reserve response [h]. \\
$R^{U, FD}_t$ (\texttt{RUFD}) & upper bound on the upwards reserve provided by flexible demand [MW]. \\
$R^{D, FD}_t$ (\texttt{RDFD}) & upper bound on the downwards reserve provided by flexible demand [MW]. \\
\\
...
Nonnegativity contraints:
\begin{equation} \label{EQ:RVarDef1}
    r^{U}_{t, \omega} \geq 0, \quad r^{D}_{t, \omega} \geq 0, \quad r^{U, B}_{t, \omega} \geq 0, \quad r^{D, B}_{t, \omega} \geq 0, \quad r^{U, FD}_{t, \omega} \geq 0, \quad r^{D, FD}_{t, \omega} \geq 0, \quad \forall u \in \mathcal{U}, t \in \mathcal{T}, w \in \Omega
\end{equation}
Components of the RM (\verb|RM_components_U| and \verb|RM_components_D|):
\begin{equation} \label{EQ:RUcomp1}
    r^{U}_{t, \omega} = r^{U, B}_{t, \omega} +  r^{U, FD}_{t, \omega} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:RDcomp1}
	r^{D}_{t, \omega} = r^{D, B}_{t, \omega} +  r^{D, FD}_{t, \omega} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
Enough energy for the demand given the RM (\verb|FlexDem_UB| and \verb|FlexDem_LB|):
\begin{equation} \label{EQ:FlexDemand_UB1}
	fd_{t, \omega} + T^R r^{D, FD}_{t, \omega} \leq \overline{D}_t \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:FlexDemand_LB1}
	\underline{D}_t \leq fd_{t, \omega} - T^R r^{U, FD}_{t, \omega} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
Bounds for the RM coming from flexible demand (\verb|FlexDemP_DReserve| and \verb|FlexDemP_UReserve|)
\begin{equation} \label{EQ:FlexDemP_DReserve1}
	r^{D, FD}_{t, \omega} \leq R^{D, FD}_t \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:FlexDemP_UReserve1}
	r^{U, FD}_{t, \omega} \leq R^{U, FD}_t \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
Bounds for the RM coming from the BESS (\verb|VPP_RU2s| and \verb|VPP_RD2s|)
\begin{equation} \label{EQ:VPP_RU2s1}
	r^{U, B} _{t, \omega} - c_{t, \omega} + d_{t, \omega}  \leq P^B \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:VPP_RD2s1}
	r^{D, B}_{t, \omega} + c_{t, \omega} - d_{t, \omega} \leq P^B \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
Bounds on the state of the batery including reserve (\verb|VPP_RU_SOCV| and \verb|VPP_RD_SOCV|):
\begin{equation} \label{EQ:VPP_RU_SOCV1}
	\underline{\SOC} \leq \soc_{t, \omega} - \frac{T^R r^{U, B}_{t, \omega}}{\eta^B E^B} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:VPP_RD_SOCV1}
	\soc_{t, \omega} + \frac{T^R r^{D, B}_{t, \omega} }{E^B} \leq \overline{\SOC} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
...
\begin{equation} \label{EQ:DM_IM_11}
	-R^{IDA} (e^{DA+}_{t, \omega} + e^{DA-}_{t, \omega})  \leq \sum_{i \in \mathcal{I}_t} e^{IM}_{i, t, \omega} \leq R^{IDA} (e^{DA+}_{t, \omega} + e^{DA-}_{t, \omega}) \quad  \forall i \in \mathcal{I},  \ \forall t \in \mathcal{T}_i, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:IM_bounds_3-41}
	-R^{IDA} (e^{DA+}_{t, \omega} + e^{DA-}_{t, \omega}) \leq e^{IM}_{i, t, \omega} \leq R^{IDA} (e^{DA+}_{t, \omega} + e^{DA-}_{t, \omega}) \quad \forall i \in \mathcal{I}, \ \forall t \in \mathcal{T}_i, \ \forall \omega \in \Omega.
\end{equation}
...
Nonnegativity contraints:
\begin{equation} \label{EQ:IBVarDef1}
    e_{t, \omega}^{IB+} \geq 0, \quad e_{t, \omega}^{IB-} \geq 0 \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}

The imbalances of the EC are defined in the following equation (\verb|Imbalances|)
\begin{equation} \label{EQ:IBDef1}
    e^{IB+}_{t, \omega} - e^{IB-}_{t, \omega} = e^{DA-}_{t, \omega} + W_{t, \omega} + PV_{t, \omega} + d_{t, \omega} \Delta T - \Big (e^{DA+}_{t, \omega} + \sum_{i \in \mathcal{I}_t} e^{IM}_{i, t, \omega} + fd_{t, \omega} + c_{t, \omega} \Delta T \Big ) \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
Control for speculative behaviour (\verb|IB_pos_UB| and \verb|IB_neg_UB|):
\begin{equation} \label{EQ:VPP_IB_MAX1}
	e_{t, \omega}^{IB+} \leq E_{t, \omega}^{IB+} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:VPP_IB_MIN1}
	e_{t, \omega}^{IB-} \leq E_{t, \omega}^{IB-} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}

\subsection{Nonanticipativity Constraints}
\label{SEC:NonanticipativityConstraints1}

For DA matched energy variables (\verb|NAC_eDA_p| and (\verb|NAC_eDA_m|):
\begin{equation} \label{EQ:NAC_eDA_p1}
	e^{DA+}_{t, \omega} = e^{DA+}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_eDA_m1}
	e^{DA-}_{t, \omega} = e^{DA-}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega} , \ \omega \neq \omega'.
\end{equation}
For DA binary variables (\verb|NAC_ieDA_p| and and \verb|NAC_ieDA_m|):
\begin{equation} \label{EQ:NAC_ieDA_p1}
	ie^{DA+}_{t, \omega} = ie^{DA+}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega} , \ \omega \neq \omega'.
\end{equation}
\begin{equation} \label{EQ:NAC_ieDA_m1}
	ie^{DA-}_{t, \omega} = ie^{DA-}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega} , \ \omega \neq \omega'.
\end{equation}
For upward reserve variables, we have (\verb|NAC_rU|, \verb|NAC_rU_B| and \verb|NAC_rU_FD|):
\begin{equation} \label{EQ:NAC_pVRU1}
	r^U_{t, \omega} = r^U_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_rUB1}
	r^{U, B}_{t, \omega} = r^{U, B}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_rUFD1}
	r^{U, FD}_{t, \omega} = r^{U, FD}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega}, \ \omega \neq \omega'.
\end{equation}

For downward reserve variables, we have (\verb|NAC_rD|, \verb|NAC_rD_B| and \verb|NAC_rD_FD|):
\begin{equation} \label{EQ:NAC_pVRD1}
	r^D_{t, \omega} = r^D_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_rDB1}
	r^{D, B}_{t, \omega} = r^{D, B}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_rDFD1}
	r^{D, FD}_{t, \omega} = r^{D, FD}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{1, \omega}, \ \omega \neq \omega'.
\end{equation}
For intraday market variables (\verb|NAC_eIM|):
\begin{equation} \label{EQ:NAC_mUC1}
	e^{IM}_{i, t, \omega} = e^{IM}_{i, t, \omega'} \quad \forall i \in \mathcal{I}, \ \forall t \in \mathcal{T}_i, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{IM}_i -1, \omega}, \ \omega \neq \omega'.
\end{equation}
For imbalance variables, we have (\verb|NAC_pIB_p| and \verb|NAC_pIB_m|):
\begin{equation} \label{EQ:NAC_pPIB1}
	e^{IB+}_{t, \omega} = e^{IB+}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_pNIB1}
	e^{IB-}_{t, \omega} = e^{IB-}_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t, \omega}, \ \omega \neq \omega'.
\end{equation}
For flexible demand variables, we have (\verb|NAC_var_fd|, \verb|NAC_var_afd_p| and  \verb|NAC_var_afd_m|):
\begin{equation} \label{EQ:NAC_var_fd1}
	fd_{t, \omega} = fd_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_var_afd_p1}
	afd^+_{t, \omega} = afd^+_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_var_afd_m1}
	afd^-_{t, \omega} = afd^-_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega}, \ \omega \neq \omega'.
\end{equation}

For BESS operational variables, we have (\verb|NAC_cV|, \verb|NAC_dV|, \verb|NAC_idV| and \verb|NAC_socV|)
\begin{equation} \label{EQ:NAC_cV1}
	c_{t, \omega} = c_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_dV1}
	d_{t, \omega} = d_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_idV1}
	id_{t, \omega} = id_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega}, \ \omega \neq \omega',
\end{equation}
\begin{equation} \label{EQ:NAC_socV1}
	\soc_{t, \omega} = \soc_{t, \omega'} \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega, \ \forall \omega' \in \mathcal{C}_{SG^{WP}_t - 1, \omega}, \ \omega \neq \omega'.
\end{equation}

\subsection{Objective Function}
\label{SEC:ObjectiveFunction1}

We define the objective function Expected Energy Community Social Welfare (EECSW) as follows:
\begin{subequations}
	\begin{align}
		EECSW & := \sum_{t \in \mathcal{T}} \sum_{\omega \in \Omega} \Pi_{\omega} \lambda^D_{t, \omega} (e^{DA+}_{t, \omega} - e^{DA-}_{t, \omega}) \label{EQ:DAincome1} \\
		& + \sum_{t \in \mathcal{T}} \sum_{\omega \in \Omega} \Pi_{\omega} \lambda^R_{t, \omega} (r^D_{t, \omega} + r^U_{t, \omega}) T^R \label{EQ:RESincome1} \\
		& + \sum_{t \in \mathcal{T}} \sum_{\omega \in \Omega} \Pi_{\omega} \sum_{i \in \mathcal{I}_t} \lambda^{I}_{i, t, \omega} e^{IM}_{i, t, \omega} \label{EQ:IMincome1} \\
		& + \sum_{t \in \mathcal{T}} \sum_{\omega \in \Omega} \Pi_{\omega} \lambda^{IB+}_{t, \omega} e^{IB+}_{t, \omega} \label{EQ:PosIBcollectRights1} \\
		& - \sum_{t \in \mathcal{T}} \sum_{\omega \in \Omega} \Pi_{\omega} \lambda^{IB-}_{t, \omega} e^{IB-}_{t, \omega} \label{EQ:NegIBPayObligations1} \\
		& - \sum_{t \in \mathcal{T}} \sum_{\omega \in \Omega} \Pi_{\omega} C^{FD} (afd^+_{t, \omega} + afd^-_{t, \omega}). \label{EQ:DemFlexPenalty1}
	\end{align}
	\label{EQ:EECSW1}
\end{subequations}

\subsection{Complete Model Formulation}
\label{SEC:CompleteModelFormulation1}
...
% Restricciones
s.t. & \eqref{EQ:FlexDemandPos1}, \eqref{EQ:DailyDem1}, \eqref{EQ:InterDem1}, \eqref{EQ:FlexDemDisplace1} & Flex. Demand Op. \\[0.2em]

& \eqref{dVdCPos1}, \eqref{EQ:VPP_state_dV1}, \eqref{EQ:VPP_state_cV1}, \eqref{EQ:SOCV1}, \eqref{EQ:socVOperLimits1}, \eqref{EQ:SOCV_ini1}, \eqref{EQ:SOCV_fin1} & BESS Op. \\[0.2em]

& \eqref{EQ:DAVarDef1}, \eqref{EQ:DA_sell_bid_LB1}, \eqref{EQ:DA_buy_bid_LB1}, \eqref{EQ:DA_buy_or_sell1}, \eqref{EQ:DA_bid_mono_11}, \eqref{EQ:DA_bid_mono_21} & EC DA \\[0.2em]

& \eqref{EQ:RVarDef1}, \eqref{EQ:RUcomp1}, \eqref{EQ:RDcomp1}, \eqref{EQ:FlexDemand_UB1}, \eqref{EQ:FlexDemand_LB1}, \eqref{EQ:FlexDemP_DReserve1}, \eqref{EQ:FlexDemP_UReserve1}, \eqref{EQ:VPP_RU2s1}, \eqref{EQ:VPP_RD2s1}, \eqref{EQ:VPP_RU_SOCV1}, \eqref{EQ:VPP_RD_SOCV1} & EC RM \\[0.2em]

& \eqref{EQ:DM_IM_11}, \eqref{EQ:IM_bounds_3-41} & EC IM \\[0.2em]

& \eqref{EQ:IBVarDef1}, \eqref{EQ:IBDef1}, \eqref{EQ:VPP_IB_MAX1}, \eqref{EQ:VPP_IB_MIN1} & EC IB \\[0.2em]

& \eqref{EQ:NAC_eDA_p1}, \eqref{EQ:NAC_eDA_m1}, \eqref{EQ:NAC_ieDA_p1}, \eqref{EQ:NAC_ieDA_m1} & NAC DA \\[0.2em]

& \eqref{EQ:NAC_pVRU1}, \eqref{EQ:NAC_rUB1}, \eqref{EQ:NAC_rUFD1}, \eqref{EQ:NAC_pVRD1}, \eqref{EQ:NAC_rDB1}, \eqref{EQ:NAC_rDFD1} & NAC RM \\[0.2em]

& \eqref{EQ:NAC_mUC1} & NAC IM \\[0.2em]

& \eqref{EQ:NAC_pPIB1}, \eqref{EQ:NAC_pNIB1} & NAC Imbalances \\[0.2em]

& \eqref{EQ:NAC_var_fd1}, \eqref{EQ:NAC_var_afd_p1}, \eqref{EQ:NAC_var_afd_m1} & NAC Flex. Demand \\[0.2em]

& \eqref{EQ:NAC_cV1}, \eqref{EQ:NAC_dV1}, \eqref{EQ:NAC_idV1}, \eqref{EQ:NAC_socV1} & NAC BESS Op. \\

\end{tabularx}
}

\subsection{\texorpdfstring{Summary of Stages for $T = 96$ (quarter-hourly)}{Summary of Stages for T = 96 (quarter-hourly)}}
...
\begin{tabular}{c c c c}
	...
	12& $fd_8, afd^{\pm}_8, c_{8, \omega}, d_{8, \omega}, id_{8, \omega}, \soc_{8, \omega}$ & $W_8, PV_8, \lambda^{IB+}_{8, \omega} \in \R$ & $p^{IB+}_{8, \omega}, p^{IB-}_{8, \omega}$ \\
    ...&...&...&...\\
	41& $fd_{41}, afd^{\pm}_{41}, c_{41, \omega}, d_{41, \omega}, id_{41, \omega}, \soc_{41, \omega}$ & $W_{41}, PV_{41}, \lambda^{IB+}_{41, \omega} \in \R$ & $e^{IB+}_{41, \omega}, e^{IB-}_{41, \omega}$\\
	42& $fd_{42}, afd^{\pm}_{42}, c_{42, \omega}, d_{42, \omega}, id_{42, \omega}, \soc_{42, \omega}$ & $W_{42}, PV_{42}, \lambda^{IB\pm}_{42, \omega} \in \R$ & $e^{IB+}_{42, \omega}, e^{IB-}_{42, \omega}$\\
    43& $fd_{43}, afd^{\pm}_{43}, c_{43, \omega}, d_{43, \omega}, id_{43, \omega}, \soc_{43, \omega}$ & $W_{43}, PV_{43}, \lambda^{IB\pm}_{43, \omega} \in \R$ & $e^{IB+}_{43, \omega}, e^{IB-}_{43, \omega}$\\
	44& $fd_{44}, afd^{\pm}_{44}, c_{44, \omega}, d_{44, \omega}, id_{44, \omega}, \soc_{44, \omega}$ & $W_{44}, PV_{44}, \lambda^{IB\pm}_{44, \omega} \in \R$ & $e^{IB+}_{44, \omega}, e^{IB-}_{44, \omega}$\\
	45& $e^{IM}_{3, t, \omega}$ & $\lambda^I_3 \in \R^{T_3}$&\\
	46& $fd_{46}, afd^{\pm}_{46}, c_{46, \omega}, d_{46, \omega}, id_{46, \omega}, \soc_{46, \omega}$ & $W_{46}, PV_{46}, \lambda^{IB\pm}_{46, \omega} \in \R$ & $p^{IB+}_{46, \omega}, p^{IB-}_{46, \omega}$ \\
	47& $fd_{47}, afd^{\pm}_{47}, c_{47, \omega}, d_{47, \omega}, id_{47, \omega}, \soc_{47, \omega}$ & $W_{47}, PV_{47}, \lambda^{IB\pm}_{47, \omega} \in \R$ & $e^{IB+}_{47, \omega}, e^{IB-}_{47, \omega}$\\
	48& $fd_{48}, afd^{\pm}_{48}, c_{48, \omega}, d_{48, \omega}, id_{48, \omega}, \soc_{48, \omega}$ & $W_{48}, PV_{48}, \lambda^{IB\pm}_{48, \omega} \in \R$ & $e^{IB+}_{48, \omega}, e^{IB-}_{48, \omega}$\\
	49& $fd_{49}, afd^{\pm}_{49}, c_{49, \omega}, d_{49, \omega}, id_{49, \omega}, \soc_{49, \omega}$ & $W_{49}, PV_{49}, \lambda^{IB\pm}_{49, \omega} \in \R$ & $e^{IB+}_{49, \omega}, e^{IB-}_{49, \omega}$\\
    ...&...&...&...\\
	98& $fd_{98}, afd^{\pm}_{98}, c_{98, \omega}, d_{98, \omega}, id_{98, \omega}, \soc_{98, \omega}$ & $W_{98}, PV_{98}, \lambda^{IB\pm}_{98, \omega} \in \R$ & $e^{IB+}_{98, \omega}, e^{IB-}_{98, \omega}$\\
	99& $fd_{99}, afd^{\pm}_{99}, c_{99, \omega}, d_{99, \omega}, id_{99, \omega}, \soc_{99, \omega}$ & $W_{99}, PV_{99}, \lambda^{IB\pm}_{99, \omega} \in \R$ &  $e^{IB+}_{99, \omega}, e^{IB-}_{99, \omega}$\\
	100& $fd_{100}, afd^{\pm}_{100}, c_{100, \omega}, d_{100, \omega}, id_{100, \omega}, \soc_{100, \omega}$ & $W_{100}, PV_{100}, \lambda^{IB\pm}_{100, \omega} \in \R$ & $e^{IB+}_{100, \omega}, e^{IB-}_{100, \omega}$\\
	101& $fd_{101}, afd^{\pm}_{101}, c_{101, \omega}, d_{101, \omega}, id_{101, \omega}, \soc_{101, \omega}$  & $W_{101}, PV_{101}, \lambda^{IB\pm}_{101, \omega} \in \R$ & $e^{IB+}_{101, \omega}, e^{IB-}_{101, \omega}$\\
	
		
	
\end{tabular}
\caption{Summary of stages, decision variables, random variable observations and recourse variables.}
\label{TABLE:stages-rv-dv-96}
\end{table}