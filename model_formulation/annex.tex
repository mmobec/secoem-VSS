...
Variable state of charge $\soc_{t, q,s}$ of the BESS (\verb|SOCV|): 
\begin{equation}
\label{EQ:SOCV2}
soc_{t,q,\omega} =
\begin{cases}
soc_{t-1,Q,\omega} + \dfrac{\Delta Q \left(pc_{t,q,\omega} - \dfrac{pd_{t,q,\omega}}{\eta_B}\right)}{E_B}, & q = 1, \\[8pt]
soc_{t,q-1,\omega} + \dfrac{\Delta Q \left(pc_{t,q,\omega} - \dfrac{pd_{t,q,\omega}}{\eta_B}\right)}{E_B}, & q = 2,\ldots,Q,
\end{cases}
\quad \forall t \in T,\; \forall \omega \in \Omega.
\end{equation}
Operational limits:
...
\begin{equation} \label{EQ:DA_bid_mono_22}
	e^{DA-}_{t, \omega} \geq e^{DA-}_{t, \omega'} \quad \forall (t,q, \omega, \omega') \in \mathcal{SSD}.
\end{equation}
...
\begin{equation} \label{EQ:DM_IM_12}
	-R^{IDA} (e^{DA+}_{t, \omega} + e^{DA-}_{t, \omega})  \leq \sum_{i \in \mathcal{I}_t} e^{IM}_{i, t, q, \omega} \leq R^{IDA} (e^{DA+}_{t, \omega} + e^{DA-}_{t, \omega}) \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega
\end{equation}
\begin{equation} \label{EQ:IM_bounds_3-42}
	-R^{IDA} (e^{DA+}_{t, \omega} + e^{DA-}_{t, \omega}) \leq e^{IM}_{i, t, \omega} \leq R^{IDA} (e^{DA+}_{t, q, \omega} + e^{DA-}_{t, \omega}) \quad \forall i \in \mathcal{I}, \ \forall t \in \mathcal{T}_i, \ \forall \omega \in \Omega.
\end{equation}
...
\begin{equation} \label{EQ:IBDef2}
 e^{IB+}_{t,q, \omega} - e^{IB-}_{t,q, \omega} =\frac{ e^{DA-}_{t, \omega}}{Q} + W_{t,q, \omega} + PV_{t,q, \omega} + d_{t,q, \omega} \Delta Q - \Big (\frac{e^{DA+}_{t, \omega}}{Q} + \sum_{i \in \mathcal{I}_t} e^{IM}_{i, t,q, \omega} + fd_{t,q, \omega} + c_{t,q, \omega} \Delta Q \Big ) \quad \forall t \in \mathcal{T}, \ \forall q \in \mathcal{Q}, \ \forall \omega \in \Omega.
\end{equation}
