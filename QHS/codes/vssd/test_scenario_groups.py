...
Gurobi required, so this can run anywhere. Ported unchanged from the HG
(15min_ren_same) branch: the grouping algorithm itself has no dependency on
the model's time-indexing convention (this mock's sgim/nSG/c values are
arbitrary small numbers regardless of branch), so nothing here differs. Only
the docstring's cross-reference (nSG=102 for QHS's real tree vs. 30 for HG's)
is branch-specific -- the mock deliberately stays small either way. Run
directly:
...
def build_mock():
    ...
    # IM2 at sgim=4 (-> sg=3), IM3 at sgim=5 (-> sg=4). nSG=5 (IB at the last
    # stage) -- deliberately kept small here regardless of branch; QHS's real
    # tree has nSG=102 (finer per-quarter-hour renewable stages), but the
    # grouping *algorithm* under test doesn't depend on that magnitude.
    sgim = {1: 3, 2: 4, 3: 5}
    nSG = 5
    S = [1, 2, 3, 4]
...
def run():
    ...
    # t=2 (RM): must be identical partition to t=1 (G_2 == G_1)
    g2 = groups[2]
