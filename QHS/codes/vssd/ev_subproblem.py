...
solution values obtained in the chain").

Ported from the HG (15min_ren_same) branch's codes/vssd/ev_subproblem.py. The
only structural difference between the two branches' models is the time index:
QHS (this branch) uses a single flat T=1..nT (nT=96 quarter-hours), whereas HG
uses a two-level (T=hour, Q=quarter) pair. Every index tuple below is therefore
one element shorter than HG's -- e.g. HG's `(t, q)` becomes this file's `(t,)`
(kept as a 1-tuple, not a bare int, so `idx + (s,)` uniformly builds the full
`(t, s)` / `(i, t, s)` variable index regardless of whether idx carries 1 or 2
free indices). Every parameter/variable/constraint name referenced here (eDA_p,
eDA_m, ieDA_p, ieDA_m, rU, rD, eIM, IM_bounds_1_pos/2/3/4, IB_pos_UB/neg_UB,
IB_pos_slack/neg_slack, PIB_p/PIB_m, FD_U, C_FD, maxTIM, EECSW) was confirmed
identical between the two branches' model_builder.py before porting.

The model shape is always built with sim_ctx.market == "DA" (mirroring HG's own
design decision, confirmed independently valid for QHS: model_builder.py, when
built with market=="DA", makes every variable across all 6 markets a free Var --
rU/rD and eIM included -- so a single "DA"-shaped instance already **is** the
paper's RP; see the recovered VSSD_implementation_report.md, sections 2.1/3.4).
This file never modifies model_builder.py, instancemanager.py, preprocessing.py,
or config_definition.py; it only reuses them as-is (InstanceManager included, for
...
# the synthetic singleton, which would trivially force them to 0 -- see
# build_ev_instance()'s docstring below).
_INSTANCE_PARAMS_T = ("pW", "pPV", "PIB_p", "PIB_m")
...
def build_ev_instance(abstract_model, rp_instance, rp_scenario_data, group):
    ...
    PIB_p/PIB_m (imbalance bid bounds) are read as the Omega_g-conditional
    expectation of the already-solved RP instance's own values, not recomputed
    on the synthetic singleton: InstanceManager.compute_imbalance_bounds()
    derives them as "deviation from the mean" (pW - mean_pW), which would be
    trivially 0 on a single averaged scenario (deviation-from-itself = 0),
    wrongly forcing pIB_p=pIB_m=0.
    """
    ...
    t_keys = [(t,) for t in range(1, nT + 1)]
    it_keys = [
        (i, t)
    ...
    # preserved S and would fail the same "index not valid" way Prob0 would if nS
    # were touched. It's dead weight here regardless: preprocessing.py only ever
    ...
    for pname in _RAW_PARAMS_T:
        cond = _cond_expectation_raw(raw[pname], prob, t_keys, omega_g, w_g)
        portal[pname] = {key + (1,): val for key, val in cond.items()}

    for pname in _RAW_PARAMS_IT:
        cond = _cond_expectation_raw(raw[pname], prob, it_keys, omega_g, w_g)
        portal[pname] = {key + (1,): val for key, val in cond.items()}

    for pname in _INSTANCE_PARAMS_T:
        param = getattr(rp_instance, pname)
        cond = _cond_expectation_instance(param, prob, t_keys, omega_g, w_g)
        portal[pname] = {key + (1,): val for key, val in cond.items()}
...
def newly_decided_at_stage(t, rp_instance):
    ...
    solved (Section 4 Step 2). Mirrors HG's ev_subproblem.newly_decided_at_stage:
    intraday-market bids become fixed at their own macro-stage, and IM3 (t=5)
    additionally fixes all recourse for quarter-hours before IM3's own delivery
    window (TIM[3]) -- those quarter-hours have "elapsed" by the time IM3 clears
    and are no longer free recourse for the IB stage.

    Every returned idx is a tuple (1 element for T-only variables, 2 for eIM's
    (i, t)) so that `idx + (s,)` always builds the correct full variable index.
    """
    T = list(rp_instance.T)
    all_t = [(t_,) for t_ in T]

    if t == 1:
        return [(v, all_t) for v in ("eDA_p", "eDA_m", "ieDA_p", "ieDA_m")]

    if t == 2:
        return [(v, all_t) for v in ("rU", "rD")]

    if t in (3, 4, 5):
        i = t - 2
        im_idx = [(i, t_) for t_ in rp_instance.TIM[i]]
        result = [("eIM", im_idx)]
        if t == 5:
            cutoff = min(rp_instance.TIM[3])
            elapsed_t = [(t_,) for t_ in T if t_ < cutoff]
            # pIB_p/pIB_m are deliberately NOT fixed here, even for elapsed
            # quarter-hours: they are the residual of the Imbalances balance
            # equation (which also involves pW/pPV), and pW/pPV are recomputed
            # fresh from each group's own Omega_g-conditional expectation at
            # every stage. If a child group's Omega_g strictly refines its
            # parent's, that expectation can shift slightly even for an
            # "elapsed" quarter-hour, and fixing pIB_p/pIB_m to the parent's
            # now-stale balance makes the equation infeasible (confirmed as a
            # real bug on HG -- see VSSD_implementation_report.md §3.4/§3.8).
            # Leaving them free lets each subproblem reconcile its own balance,
            # the correct behaviour for a computed residual rather than a
            # genuinely memorized decision.
            for v in (
                "var_fd", "var_afd_p", "var_afd_m", "dV", "cV", "idV",
                "rU_B", "rD_B", "rU_FD", "rD_FD",
            ):
                result.append((v, elapsed_t))
            # socV is declared over T0 x S (no separate quarter dimension --
            # model_builder.py:378), and SOCV_ini_rule only ever pins
            # socV[T0.first(), s] == SOCini (model_builder.py:639-642), i.e. the
            # day-boundary state socV[0, s]. Unlike HG (whose socV carries a Q
            # dimension and only q=1 is ever constrained), there is no
            # unconstrained sibling index to accidentally include here.
            soc_idx = elapsed_t + [(0,)]
...
def relax_im_bounds_for_fixed_da(instance):
    ...
    (model_builder.py:728-765). That formula is a Python closure over
    ...
    branch of _cap20, with no fallback for a ~0 fixed DA position:
        cap = maxTIM * (eDA_p[t,s] + eDA_m[t,s])
    Once eDA is fixed to a real number (t>=2) and that number is ~0 at some t --
    an entirely ordinary outcome, plenty of quarter-hours clear nothing in DA --
    cap collapses to exactly 0 regardless of maxTIM's value, forcing
    eIM[.,t,.] == 0 there. If the group's own Omega_g-conditional-average
    forecast needs any nonzero imbalance at that quarter-hour, the subproblem is
    flatly infeasible -- the identical failure mode HG hit and fixed (see
    VSSD_implementation_report.md §3.4).

    The real (would-be) sequential pipeline never hits this: once
    sim_ctx.market != "DA", _cap20()'s else branch falls back to FD_U[t]
    (flexible-demand upper bound, much larger) whenever the fixed DA position is
    ~0. That branch is unreachable from this package's single "DA"-shaped
    abstract model, so it's reproduced here directly on the instance instead:
    deactivate the four baked-in constraints and re-add them with the same
    D>EPS-else-FD_U formula, reading eDA_p/eDA_m as plain fixed numbers.
    ...
    mirroring HG, RM's own group partition coincides with DA's (see
    ...
    def _cap(m, t, s):
        D = value(m.eDA_p[t, s]) + value(m.eDA_m[t, s])
        return m.maxTIM * D if D > EPS else m.FD_U[t]

    def IM_bounds_1_pos_rule(m, t, s):
        return sum(
            m.eIM_pos[i, t, s] + m.eIM_neg[i, t, s] for i in m.IMT[t]
        ) <= _cap(m, t, s)
    instance.IM_bounds_1_pos_relaxed = pyo.Constraint(
        instance.T, instance.S, rule=IM_bounds_1_pos_rule
    )
    ...
    instance.IM_bounds_2_relaxed = pyo.Constraint(
        instance.T, instance.S, rule=IM_bounds_2_rule
    )

    def IM_bounds_3_rule(m, i, t, s):
        return m.eIM[i, t, s] >= -_cap(m, t, s)
    instance.IM_bounds_3_relaxed = pyo.Constraint(
        instance.IM, instance.T, instance.S, rule=IM_bounds_3_rule
    )

    def IM_bounds_4_rule(m, i, t, s):
        return m.eIM[i, t, s] <= _cap(m, t, s)
    instance.IM_bounds_4_relaxed = pyo.Constraint(
        instance.IM, instance.T, instance.S, rule=IM_bounds_4_rule
    )

    # Price the FD_U-fallback eIM usage at C_FD (model_builder.py:79), the
    # model's own existing, active flexible-demand cost rate -- the same rate
    # that already prices var_afd_p/var_afd_m. eIM enters EECSW as pure revenue;
    # an unpriced FD_U-sized cap would let the optimizer manufacture free
    # revenue at every (t,s) where the fallback fires, not just escape the
    # spurious infeasibility. Only the fallback branch (D<=EPS) is priced --
    # when D>EPS, cap=maxTIM*D is the model's own real rule, untouched, no
    # penalty.
    penalty_eIM = sum(
        value(instance.Prob[s]) * value(instance.C_FD)
        * (instance.eIM_pos[i, t, s] + instance.eIM_neg[i, t, s])
        for t in instance.T
        for s in instance.S
        if value(instance.eDA_p[t, s]) + value(instance.eDA_m[t, s]) <= EPS
...
# Matches model_builder.py's own model.imbalanceSlackPenalty default (100) --
# that Param is only ever declared in the RM/"IM" market branch
# (model_builder.py:822), never under market="DA", so it doesn't exist on any
# VSSD instance to read from.
...
def relax_ib_bounds_for_fixed_da(instance):
    ...
    (model_builder.py:819-826), with no slack -- vs. market in {"RM","IM*"},
    which relaxes to PIB_p/PIB_m + IB_pos_slack/IB_neg_slack (model_builder.py:
    828-836). Every EV_g here is built under market="DA" (see module docstring),
    ...
    compute_imbalance_bounds(): PIB_p[t,s] = max(0, pW[t,s]+pPV[t,s] -
    mean_pW[t]-mean_pPV[t]), i.e. the *positive part* of a scenario's deviation
    from the population-mean renewable output). PIB_p[t,s]=0 whenever that
    scenario sits at or below the mean at that quarter-hour -- not a rare edge
    case but the routine outcome for roughly half of all scenario/quarter-hour
    pairs, and for a singleton group (|Omega_g|=1) there's no averaging to
    smooth it away: PIB_p for the group *is* PIB_p for that one scenario. RP
    never hits this because it solves every scenario's full joint recourse
    (eIM, battery, flexible demand) with nothing pre-fixed, so it always has
    some other way to keep pIB_p<=0 when PIB_p=0; an EV_g downstream in the
    fixing chain may not, once earlier stages' fixed values have already used
    up that recourse. (Same root cause HG diagnosed via Gurobi IIS -- see
    VSSD_implementation_report.md §3.4.)
    ...
    `self.model != "DA"` (model_builder.py:386), which compares the
    ...
    added in a maximization). Relaxing its cap without pricing IB_pos_slack
    would let the optimizer manufacture up to 200 units of free revenue
    whenever profitable, not just enough to escape a spurious infeasibility --
    silently inflating Z_EV^g/EDEV_t rather than reporting a genuine economic
    estimate (IB_neg_slack is self-limiting even unpenalized, since pIB_m is a
    cost term, but both are penalized here to match model_builder.py's own
    design intent for the RM/IM branches). EECSW is deactivated and recreated
    ...
    def IB_pos_UB_relaxed_rule(m, t, s):
        return m.pIB_p[t, s] <= value(m.PIB_p[t, s]) + m.IB_pos_slack[t, s]
    instance.IB_pos_UB_relaxed = pyo.Constraint(
        instance.T, instance.S, rule=IB_pos_UB_relaxed_rule
    )

    def IB_neg_UB_relaxed_rule(m, t, s):
        return m.pIB_m[t, s] <= value(m.PIB_m[t, s]) + m.IB_neg_slack[t, s]
    instance.IB_neg_UB_relaxed = pyo.Constraint(
        instance.T, instance.S, rule=IB_neg_UB_relaxed_rule
    )

    penalty = sum(
        value(instance.Prob[s]) * _IB_SLACK_PENALTY_RATE *
        (instance.IB_pos_slack[t, s] + instance.IB_neg_slack[t, s])
...
def snapshot_solution(instance):
    ...
    Some Var indices are structurally never referenced by any constraint or the
    objective, so Gurobi never assigns them a value -- skip those rather than
    let value() raise. Safe to skip: apply_fixed_values() only ever looks up the
    specific (var, idx) pairs newly_decided_at_stage() lists, never a full
    variable, so a skipped index is simply never requested.
    """
    result = {}
...
def build_full_instance(sim_ctx, abstract_model, rp_scenario_data, rp_instance,
    ...
    not substituted by a conditional average. compute_instance() also calls
    fix_eIM(), which is a no-op for sim_ctx.market=="DA" (the only market this
    package ever uses -- instancemanager.py:406-407).
    """
    raw = rp_scenario_data.data()
    prob = {s: value(rp_instance.Prob[s]) for s in rp_instance.S}
    ...
    for pname in _RAW_PARAMS_T + _RAW_PARAMS_IT:
        portal[pname] = {
...
def _parse_iis_scenarios(ilp_path):
    ...
    DA_bid_mono_1/2 (and any other "*mono*"-named constraint) encodes two
    scenario indices directly in its own name (the last two underscore-
    separated tokens, l and k) -- both are extracted even when Gurobi's
    presolve has eliminated every variable from that row (e.g. both endpoints
    already fixed), leaving nothing else in the file to name them.
    """
    scenarios = set()
    active = False
